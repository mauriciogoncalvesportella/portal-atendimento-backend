...
class AssemblerQueryService {
    ...
    createMany(items) {
        ...
        const converted = items.map((c) => assembler.convertToEntity(c));
        return this.assembler.convertAsyncToDTOs(this.queryService.createMany(converted));
    }
    createOne(item) {
        const c = this.assembler.convertToEntity(item);
    ...
    updateMany(update, filter) {
        return this.queryService.updateMany(this.assembler.convertToEntity(update), 
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        this.assembler.convertQuery({ filter }).filter);
    }
    updateOne(id, update, opts) {
        return this.assembler.convertAsyncToDTO(this.queryService.updateOne(id, this.assembler.convertToEntity(update), this.convertFilterable(opts)));
    }
