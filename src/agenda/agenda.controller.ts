...
import {
  EventoAddDTO,
  TipoAgendamentoAddDTO,
  TipoAgendamentoUpdateDTO,
  EventoUpdateDTO,
  EventoDuplicarDTO,
  EventoFixarDTO,
} from "./agenda.dto";
...
export class AgendaController {
  ...
  async eventoAll(
    @Request() req,
    @Query("userTarget") userTarget: number,
    @Param("dtinicio") dtinicio: string,
    @Param("dtfim") dtfim: string
  ) {
  ...
  async tipoAgendamentoAdd(
    @Body(new ParseArrayPipe({ items: TipoAgendamentoAddDTO }))
    data: TipoAgendamentoAddDTO[]
  ): Promise<void> {
  ...
  async tipoAgendamentoUpdate(
    @Body(new ParseArrayPipe({ items: TipoAgendamentoUpdateDTO }))
    data: TipoAgendamentoUpdateDTO[]
  ): Promise<void> {
